...
print('Количество значений за пределами границ выбросов в "bki_request_cnt":',        len(train_data.loc[train_data['bki_request_cnt'] > 7.5]))
print((len(train_data.loc[train_data['bki_request_cnt'] > 7.5]) /train_data.shape[0])*100, '% общего датасета')

print('Количество значений за пределами границ выбросов в "decline_app_cnt":',        len(train_data.loc[train_data['decline_app_cnt'] > 2]))
print((len(train_data.loc[train_data['decline_app_cnt'] > 2]) /train_data.shape[0])*100, '% общего датасета')
 
print('Количество значений за пределами границ выбросов в "income":',        len(train_data.loc[train_data['income'] > 90_000]))
print((len(train_data.loc[train_data['income'] > 90_000]) /train_data.shape[0])*100, '% общего датасета')

print('Количество значений за пределами границ выбросов в "score_bki":',        len((train_data.loc[train_data['score_bki'] > -0.52907]) + (train_data.loc[-3.29925 > train_data['score_bki']]))) 
